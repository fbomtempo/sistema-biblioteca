...
export class EmprestimoListaComponent implements OnInit {
  ...
  readonly columns: PoTableColumn[] = [
    ...
    },
    {
      ...
      labels: [
        {
          value: SituacaoEmprestimo.PENDENTE,
          color: 'color-08',
          label: 'Pendente',
        },
        {
          value: SituacaoEmprestimo.FINALIZADO,
          color: 'color-11',
          label: 'Finalizado',
        },
      ],
  ...
  @ViewChild(ModalExclusaoComponent, { static: true })
  modalExclusao!: ModalExclusaoComponent;
  ...
  private atualizarLista(): void {
    this.emprestimos$ = this.emprestimoService.recuperarTodos().pipe(
      map((emprestimos) => {
  ...
  mostrarDados(emprestimos: Emprestimo[]): Emprestimo[] {
    ...
    return emprestimos.filter((emprestimo) => {
      return emprestimo.cliente.nome
        .replace(/\s/g, '')
        .toLowerCase()
        .includes(this.filtro.replace(/\s/g, '').toLowerCase());
    });
  }
  ...
  deletar(res: any): void {
    if (res.resposta == true) {
      this.emprestimoService.remover(res.obj.id).subscribe({
        complete: () => {
